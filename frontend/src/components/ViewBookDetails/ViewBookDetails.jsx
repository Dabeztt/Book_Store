...
const ViewBookDetails = () => {
  ...
  const handleFavourite = async () => {
    const response = await axios.put(
      "https://book-store-yacq.onrender.com/api/v1/them-vao-yeu-thich",
      {},
      { headers }
    );
  ...
  const handleCart = async () => {
    const response = await axios.put(
      "https://book-store-yacq.onrender.com/api/v1/them-vao-gio-hang",
      {},
      { headers }
    );
    alert(response.data.message);
  ...
  const deleteBook = async () => {
    const response = await axios.delete(
      "https://book-store-yacq.onrender.com/api/v1/xoa-sach",
      { headers }
    );
    navigate("/sach");
    alert(response.data.message);
      ...
      {Data && (
        <div className="px-4 md:px-12 py-8 bg-zinc-900 flex flex-col lg:flex-row gap-8 items-start">
            ...
            <div className="flex flex-col lg:flex-row justify-around bg-zinc-800 p-12 rounded">
              ...
              {isLoggedIn === true && role === "admin" && (
                <div className="flex flex-col md:flex-row lg:flex-col items-center justify-between lg:justify-start mt-8 lg:mt-0">
                  ...
                  <button
                    className="text-red-500 rounded md:mt-0 lg:rounded-full text-3xl p-3 mt-8 md:mt-0 lg:mt-8 bg-white flex items-center justify-center"
                    onClick={deleteBook}
                  >
                    <MdOutlineDelete />
          ...
          <div className="p-4 w-full lg:w-3/6">
            ...
            <p className="text-zinc-400 mt-1">Bởi {Data.author}</p>
